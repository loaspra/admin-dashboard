...
export async function createRowProduct(productData: any, detailData: any) {
  ...
  const detailModelName = getDetailModelName(
...
export async function updateRowProduct(
  productId: string,
  productData: any,
  detailData: any,
) {
  ...
  const detailsModelName = getDetailModelName(
    productData.productType,
  ) as PrismaModelName;
...
export async function initializeStorage() {
  ...
  if (typeof window !== "undefined" || !supabaseAdmin) {
    console.error(
      "initializeStorage should only be called from server-side code",
    );
    return false;
  }
