...
`;
          ...
          const categorias = {
            deportes: {
              collections: ["boxing", "judo", "gym", "bjj", "futbol", "messi"],
            },
            musica: {
              collections: ["harry styles", "arctick monkeys", "bad bunny"],
            },
            ...
            arte: {
              collections: ["psicodelicos", "art", "obras de arte", "estatuas"],
            },
            cosmos: { collections: ["astronomia"] },
            profesiones: {
              collections: ["med", "programadores", "data", "security"],
            },
            caricaturas: {
              collections: [
                "snoopy",
...
Mas contexto: las imagenes son planchas de stickers, pueden tener diferentes tematicas y colores.`;
    ...
    try {
      ...
      const product = await prisma.product.create({
        data: {
          ...
          categoryId:
            categoryId ||
            (await this.getOrCreateCategoryId(metadata.categoria)),
          collectionId:
            collectionId ||
            (metadata.coleccion
              ? await this.getOrCreateCollectionId(
                  metadata.categoria,
                  metadata.coleccion,
                )
              : null),
      ...
      switch (productType) {
        case "cap":
          await prisma.capDetails.create({
            data: {
              id: uuidv4(),
              sizes: ["Adult Unisex"],
              colors: [metadata.color || "various"],
              material: "Cotton Blend",
              adjustable: true,
              style: metadata.estilo,
        ...
        default:
          ...
          console.warn(
            `Product details not created for unknown type: ${productType}`,
          );
      }
      ...
      console.error(
        `Error in processAndUploadImageWithType for ${fileName}:`,
        error,
      );
  ...
  // New method to process image and return data for review instead of saving directly
  static async processImageForReview(
    imageBuffer: Buffer,
    fileName: string,
    productType: string,
    categoryId?: string,
    ...
    try {
      ...
      const [metadata, publicUrl] = await Promise.all([
        ...
        this.uploadImageToSupabase(optimizedBuffer, fileName, productType),
      ]);
      ...
      return {
        id: productId,
        name: metadata.nombre || "Untitled Product",
        description: metadata.tags?.join(", ") || "No description",
      ...
      console.error(`Error in processImageForReview for ${fileName}:`, error);
      throw error;
    }
  }
  ...
  static async processAndUploadImageWithCustomCategorization(
    ...
    console.warn(
      "processAndUploadImageWithCustomCategorization has been deprecated. Use processAndUploadImageWithType with category and collection parameters instead.",
    );
    return this.processAndUploadImageWithType(
      imageBuffer,
      fileName,
      productType,
      categoryId,
  ...
  static async processAndUploadImage(
    ...
    console.warn(
      "Calling deprecated processAndUploadImage. Use processAndUploadImageWithType or processJsonImages.",
    );
  ...
  static async processBatchImagesWithType(
    ...
    if (!isValidProductType(productType)) {
      ...
      throw new Error(
        `Invalid product type provided for batch processing: ${productType}`,
      );
    }
    ...
    for (const file of files) {
      try {
        ...
        console.error(
          `Error processing ${file.originalname} for type ${productType}:`,
          error,
        );
  ...
  static async processBatchImages(files: UploadedFile[]): Promise<string[]> {
    console.warn(
      "Calling deprecated processBatchImages. Use processBatchImagesWithType.",
    );
    // Defaulting to 'stickerSheet'
    return this.processBatchImagesWithType(files, "stickerSheet");
  }

  static async processJsonImages(
