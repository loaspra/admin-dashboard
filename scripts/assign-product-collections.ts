...
class RateLimiter {
  ...
  private async processQueue() {
    ...
    const timeUntilNextSlot = Math.max(
      0,
      this.windowMs - (now - oldestTimestamp),
    );
...
class ProductCollectionAssigner {
  ...
  constructor(dryRun: boolean = true) {
    ...
    if (!apiKey) {
      throw new Error(
        "GOOGLE_AI_API_KEY or GOOGLE_API_KEY environment variable is required",
      );
    }
  ...
  private async saveUncategorizedProducts(
    uncategorized: UncategorizedProduct[],
  ): Promise<void> {
    ...
    const report = {
      metadata: {
        ...
        summary: {
          no_collections_available: uncategorized.filter(
            (p) => p.reason === "no_collections_available",
          ).length,
          ai_suggested_none: uncategorized.filter(
            (p) => p.reason === "ai_suggested_none",
          ).length,
          ai_suggested_invalid: uncategorized.filter(
            (p) => p.reason === "ai_suggested_invalid",
          ).length,
          processing_error: uncategorized.filter(
            (p) => p.reason === "processing_error",
          ).length,
        },
        categoriesWithIssues: [
          ...new Set(uncategorized.map((p) => p.category)),
        ].sort(),
      },
    ...
    try {
      ...
      console.log(
        `📝 Saved ${uncategorized.length} uncategorized products to: ${filename}`,
      );
      console.log(`📊 Breakdown:`);
      console.log(
        `   - No collections available: ${report.metadata.summary.no_collections_available}`,
      );
      console.log(
        `   - AI suggested none: ${report.metadata.summary.ai_suggested_none}`,
      );
      console.log(
        `   - AI suggested invalid: ${report.metadata.summary.ai_suggested_invalid}`,
      );
      console.log(
        `   - Processing errors: ${report.metadata.summary.processing_error}`,
      );
    } catch (error) {
  ...
  async getProductsWithoutCollections(): Promise<ProductToProcess[]> {
    ...
    if (this.dryRun && products.length > 0) {
      console.log(
        `🧪 DRY RUN: Limited to ${products.length} products for testing`,
      );
    }
  ...
  async getAllCollectionsFlat(): Promise<
    Array<{ id: string; name: string; category: string; categoryId: string }>
  > {
    const collections = await this.prisma.collection.findMany({
      select: {
        id: true,
  ...
  private async suggestCollectionForProduct(
    ...
    const categoryCollections =
      availableCollections[product.categoryName] || [];
    const allCollections = await this.getAllCollectionsFlat();

    if (categoryCollections.length === 0) {
      console.log(
        `⚠️  No collections available for category: ${product.categoryName}`,
      );
      uncategorized.push({
        id: product.id,
        name: product.name,
    ...
    const allCollectionsByCategory = allCollections.reduce(
      (acc, col) => {
...
${Object.entries(allCollectionsByCategory)
...
4. Si realmente no hay ninguna apropiada, sugiere "ninguna" como colección y "cambio_categoria": false`;
    ...
    return geminiRateLimiter.execute(async () => {
      try {
        console.log(
          `🤖 Analyzing product: ${product.name} (${product.categoryName})`,
        );
        const rateLimitStatus = this.getRateLimitStatus();
        console.log(
          `   📊 Rate limiter status: ${rateLimitStatus.currentRate}/${rateLimitStatus.maxRate} requests, queue: ${rateLimitStatus.queueLength}`,
        );
        ...
        if (aiResponse.coleccion === "ninguna") {
          console.log(
            `⚠️  AI couldn't find appropriate collection for ${product.name}`,
          );
          uncategorized.push({
            id: product.id,
            name: product.name,
        ...
        if (aiResponse.cambio_categoria && aiResponse.categoria) {
          ...
          const targetCategoryCollections =
            availableCollections[targetCategory] || [];
          suggestedCollection = targetCategoryCollections.find(
            (c) => c.name.toLowerCase() === aiResponse.coleccion.toLowerCase(),
          );
        } else {
          // Look in current category first
          suggestedCollection = categoryCollections.find(
            (c) => c.name.toLowerCase() === aiResponse.coleccion.toLowerCase(),
          );
        }
        ...
        if (!suggestedCollection) {
          console.log(
            `⚠️  AI suggested "${aiResponse.coleccion}" but it's not found in any category`,
          );
          uncategorized.push({
            id: product.id,
            name: product.name,
        ...
        if (categoryChange) {
          console.log(
            `🔄 Category change suggested: "${product.name}" ${categoryChange.from} → ${categoryChange.to}`,
          );
        }
  ...
  async processProducts(): Promise<ProcessingResult> {
    ...
    console.log(
      `📚 Available collections across ${Object.keys(availableCollections).length} categories\n`,
    );
    ...
    for (const product of products) {
      try {
        const suggestion = await this.suggestCollectionForProduct(
          product,
        ...
        if (suggestion) {
          ...
          errors.push(
            `Could not suggest collection for product: ${product.name}`,
          );
        }
      } catch (error) {
  ...
  private printResults(result: ProcessingResult) {
    console.log("\n" + "=".repeat(60));
    console.log(
      `📊 COLLECTION ASSIGNMENT SUMMARY (${this.dryRun ? "DRY RUN" : "APPLIED"})`,
    );
    ...
    if (result.suggestions.length > 0) {
      ...
      Object.entries(suggestionsByCategory).forEach(
        ([category, suggestions]) => {
          ...
          suggestions.forEach((suggestion) => {
            ...
            console.log(
              `    Reasoning: ${suggestion.suggestedCollection.reasoning}`,
            );
            if (suggestion.categoryChange) {
              console.log(
                `    🔄 Category Change: ${suggestion.categoryChange.from} → ${suggestion.categoryChange.to}`,
              );
              console.log(
                `    📝 Change Reason: ${suggestion.categoryChange.reason}`,
              );
            }
            console.log(
              `    Tags: [${suggestion.currentInfo.tags.join(", ")}]`,
            );
            console.log();
          });
        },
    ...
    if (result.uncategorized.length > 0) {
      ...
      console.log("-".repeat(40));
      const uncategorizedByReason = result.uncategorized.reduce(
        (acc, product) => {
    ...
    if (this.dryRun) {
      console.log(
        "💡 This was a DRY RUN. To apply these changes, run with --apply flag.",
      );
    } else {
      console.log(
        "✅ Collection assignments have been applied to the database.",
      );
    }
  }
  ...
  async applyAssignments(suggestions: CollectionSuggestion[]): Promise<void> {
    ...
    try {
      ...
      for (const suggestion of suggestions) {
        if (suggestion.categoryChange) {
          console.log(
            `  🔄 "${suggestion.currentInfo.name}": ${suggestion.categoryChange.from} → ${suggestion.categoryChange.to}`,
          );
          ...
          if (!targetCategory) {
            console.error(
              `❌ Target category "${suggestion.categoryChange.to}" not found`,
            );
            continue;
          }
          ...
          console.log(
            `  📄 Assigning "${suggestion.currentInfo.name}" → ${suggestion.suggestedCollection.name}`,
          );
